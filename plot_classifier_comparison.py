...
for ds_cnt, ds in enumerate(datasets):
    ...
    for name, clf in zip(names, classifiers):
        ...
        tpr=dict()
        roc_auc=dict()
        for roc_ind in range(len(y)):
            fpr[roc_ind], tpr[roc_ind], _ = roc_curve(y_test,proba)
            roc_auc[roc_ind] = auc(fpr[roc_ind], tpr[roc_ind])

        
        plot_x_coord=0 if plot_x >= 5 else 1
        _, plot_y_coord=divmod(plot_y,5)
        axarr[plot_x_coord,plot_y_coord].plot(fpr[2],tpr[2],lw=2,label='AUC = %0.2f' % roc_auc[2])
